...
<table class="table table-bordered tableCompare" >
        ...
        <tr>
            ...
            <c:forEach items="${tableCompare}" var="list_color" varStatus="counterColor">
                <td id="color<%=counterColor%>" style="text-align:center;">${list_color.colorCoating}</td>
                <%  counterColor++; %>
            </c:forEach>
        </tr>
        <tr>
            <td>Water resistance (FK or -)</td>
            <% int counterWaterResistance = 0; %>
            <c:forEach items="${tableCompare}" var="list_water_resistance" varStatus="counterWaterResistance">
                <td id="waterResistance<%=counterWaterResistance%>" style="text-align:center;">
                <c:choose>
                    <c:when test="${list_water_resistance.waterResistance}">
                        FK
                    </c:when>
                    <c:when test="${!list_water_resistance.waterResistance}">
                        -
        ...
        <tr>
            ...
            <c:forEach items="${tableCompare}" var="list_sanded" varStatus="counterSanded">
                <td id="sanded<%=counterSanded%>" style="text-align:center;">
                <c:choose>
